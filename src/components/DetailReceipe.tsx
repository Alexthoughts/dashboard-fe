...
export const DetailReceipe: FC<DetailReceipe> = ({ className, onChangeRecipeImage, circleName }) => {
        ...
        if (!nextUpdateAt || dayjs() >= nextUpdateAt) {
            receipesApiCall()
                ...
                .finally(() => {
                    ...
                    setIsFirstRender(true);
                    const favoriteRecipesFromLocalStorage = localStorage.getItem(localStorageKeyFavourite);
    ...
    const handleDeleteSavedRecipe = (uri: string) => {
        const newListOfSavedRecipes = listOfSavedReceipes.filter((recipe) => recipe.uri != uri);
    ...
    useEffect(() => {
        const isRecipeSaved = listOfSavedReceipes.some((recipe) => recipe.uri === renderedReceipe?.uri);
    ...
    if (renderedReceipe) {
        return (
            ...
            >
                    ...
                    >
                        {isFavouriteRecipe ? (
                            ...
                            <IconButton
                                ...
                                onClick={saveRecipe}
                            >
                                <FavoriteBorderIcon fontSize="inherit" sx={{ color: 'rgba(255, 214, 0, 0.5)' }} />
                            </IconButton>
                        )}
                        <Box sx={{ display: 'flex' }}>
                ...
                <ul style={{ marginBottom: '10px', minHeight: '3.5rem', overflowY: 'auto' }}>{ingredients}</ul>
                ...
                <Box sx={{ display: 'flex', gap: '5px', marginBottom: '1rem' }}>
                    ...
                    <Chip
                        label={`${renderedReceipe.mealType?.[0]}${
                            renderedReceipe.mealType?.length > 1 ? `/${renderedReceipe.mealType?.[1]}` : ''
                        }`}
                        sx={{
                    ...
                    <Chip
                        label={`${renderedReceipe?.dishType?.[0]}${
                            renderedReceipe?.dishType?.length > 1 ? ` and ${renderedReceipe?.dishType?.[1]}` : ''
                        }`}
                        sx={{
